...
构造函数读取参数、建立 TF 缓存、创建订阅/发布和固定频率安全监测定时器。消息先后到达不固定；节点每次检查起点、目标、地图、TF、里程计及交通数据是否可用。缺少条件时不盲目规划。默认事件模式下，静态水域首次规划后只轻量检查并刷新同一条路径，不重复搜索。
...
默认采用事件规划。timer每秒继续更新感知、CPA、COLREG状态并轻量检查剩余轨迹，但不会每秒运行搜索；新目标、路径被新静态障碍阻断、进入目标船预测碰撞域、违反当前会遇规则或偏航过大时才重新规划。安全轨迹会原样刷新消息时间戳交给DWA持续执行，不改变轨迹点。附近有船但与路径不冲突时不重新规划。动态风险下若确实无安全解，Hybrid A*发布空路径让DWA停车并在后续监测周期重试。
...
平滑不只是把小段折线变成圆弧。相邻点平滑项减少局部锯齿，long_scale平滑项减少较长距离的左右波浪。发生事件重规划时，旧路径仍作为可被安全覆盖的软参考。如果旧路径进入有效会遇的预测7米软域，本次搜索自动关闭旧路径吸引；新安全路径随后由DWA持续执行，不再因每秒重搜而左右换边。它不是硬走廊，障碍和COLREG硬约束始终优先。

每轮平滑后重新采样检查岸线、独立障碍、时间同步动态船和避让方向；失败会缩小幅度或退回当前有效路径。此检查不等于完整船舶动力学或平滑后曲率上限认证，实船还需要验证操纵性能。

建议一次只改一个量：跨周期近端跳线先调near_field_reference_weight；希望稳定区更长再调near_field_reference_horizon；必要绕行显得迟钝时增大near_field_deviation_scale或降低权重。单条路线整体波浪多调长尺度平滑项，局部锯齿调普通平滑项。更大稳定/平滑权重并不保证任何地形都更好，过大可能延迟软层面的路线优化，硬安全检查仍不会被覆盖。

## 5. 会遇状态与风险圈

DCPA 是匀速匀向假设下最近会遇距离，TCPA 是到达最近点的时间。TCPA<0 通常表示最近会遇已过去；DCPA 不是当前两船距离。风险综合 DCPA 与“当前距离或 TCPA”，不应只看一个圈判断。

当前MONITOR保持AIS式远程预警：80米/40秒、DCPA 10米。ACTION缩为24米/16秒、DCPA 5米；EMERGENCY为8米/6秒、DCPA 2.5米。距离和TCPA在风险判定中为OR，并同时要求DCPA门槛；这些细圈不是传感器截止范围、软规划域或停车圈。

当前按小型无人船等效半径0.5+0.5米、冗余1米，普通中心硬净空为2米，追越为3米。有实际碰撞风险时Hybrid在7米软域增加代价，保留5米规划调整带；事件模式首次会遇由DCPA/TCPA提前触发，已成功规划后仅在剩余路径重新进入2米预测硬域时再次搜索。默认tracking_only的DWA不再执行2米目标船停车圈。对遇锁定期间还检查完整右舷通过几何。4m/s下2米不足以作为物理制动起点，安全仍依赖CPA预测和提前规划。
...
| `setNearFieldStabilityParams` | 配置本船近前方跨周期稳定软代价、衰减距离和偏离尺度；障碍/COLREG硬约束优先。 |
| `setHeadOnStarboardEnforced` | 开关对遇完整右舷通过约束；正常开启，仅右侧走廊完全无解时由节点临时关闭进行安全回退。 |
...
| `search` | 执行一次时空Hybrid A*：探测恢复方向、构建启发与参考场、扩展运动基元/近端Dubins连接并回溯；由事件调度器按需调用。 |
| `remainingPathNeedsReplan` | 轻量采样剩余路径，检查静态阻断、预测动态碰撞、COLREG方向和横向偏航；返回true才重新搜索。 |
...
| `headOnStarboardPassAllowed` | 对遇未通过时检查候选是否持续位于预测目标右舷侧且未越过原航向左转；目标落到船尾安全域后放行恢复。 |
...
| `HybridAStarNode` | 加载参数并建立TF/话题；timer持续监测风险和刷新缓存路径，只有事件触发时执行局部搜索。 |
...
| `goalCallback` | 接收活动目标并使缓存路径失效；下一次planningTimerCallback触发一次搜索，本回调不直接驱动本船。 |
...
| `handlePlanningFailure` | 静态偶发失败保留显示；动态交通相关失败立即发空路径使DWA本周期停车，禁止沿未重新验证的旧直线继续数秒。 |
