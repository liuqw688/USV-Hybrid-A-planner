...
Hybrid A* 保持固定频率持续搜索和发布，不直接复用上一条规划结果：

- 上一条有效航线只生成软参考距离层，让相邻周期避免无意义的大幅跳变；
...
- 障碍物或目标船出现时仍在下一个规划周期重新搜索，连续性代价可以被安全规则覆盖。

平滑功能仅修改Hybrid A*；本次速度/安全联调另对DWA加入8m停车和终点制动限制。
...
船体与避碰域现在为：本船半径1.5m、目标船半径2m、冗余4m，中心间硬距离
7.5m；追越硬距离9m；青色软域9m；当前距离小于8m发布停车命令。
目标船显示长1m、宽1.5m，其2m避碰半径是独立保守包络，不等于显示尺寸。
...
带的比例为 0；之后连续 8 次重规划全部发布非空路径，首次成功后空路径数量为 0，
RViz 不再周期性闪烁。

轨迹稳定采用两层保护：静态场景不创建重复时间状态，并使用二维水域引导搜索
穿过弯曲河道；新目标只清空旧路径一次，之后 RViz 保留最近有效轨迹。DWA 的
`path_stale_timeout` 为 3 秒，若规划器持续没有新轨迹，控制器仍会停车。

后方目标测试中，船体实际航迹向仍为 123° 时，策略航向会立即切换为 -57°并
报告 `reverse_intent=true`，证明航道角色在船体完成掉头前切换。

轨迹平滑测试会比较原始与后处理轨迹的长度、累计转向波动和安全性；运行节点仍按
`planner_frequency` 固定频率执行完整搜索，不使用静态路径复用。

整链路连续运行测试以约 1 Hz 收到 12 条非空规划；每条路线的起点均随船位更新，
证明每周期确实重新搜索。相邻规划平均变化约 0.62 m、最大约 0.79 m；最终路线为
87 点，中位点距约 0.99 m，说明输出已统一采样且周期间没有大幅跳线。
